...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment,
        ...
        updateCreditHistory(payment, creditHistories, TransanctionType.DEBIT);
    ...
    public PaymentEvent validateAndCancelPayment(Payment payment,
        ...
        updateCreditHistory(payment, creditHistories, TransanctionType.CREDIT);
        if (failureMessages.isEmpty()) {
    ...
    private void updateCreditHistory(Payment payment, List<CreditHistory> creditHistories, TransanctionType transanctionType) {
    ...
    private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        Money totalCreditHistory = getTotalHistoryAmount(creditHistories, TransanctionType.CREDIT);
        Money totalDebitHistory = getTotalHistoryAmount(creditHistories, TransanctionType.DEBIT);
        if (totalDebitHistory.isGreaterThan(totalCreditHistory)) {
            log.error("Customer with id: {} doesn't have enough credit according to the credit history", creditEntry.getCustomerId().getValue());
            failureMessages.add("Customer with id: " + creditEntry.getCustomerId().getValue() + " doesn't have enough credit according to the credit history");
        }
    ...
    private Money getTotalHistoryAmount(List<CreditHistory> creditHistories, TransanctionType transanctionType) {
        return creditHistories.stream().filter(creditHistory -> creditHistory.getTransanctionType() == transanctionType)
